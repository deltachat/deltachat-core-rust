...
pub fn dc_chat_load_from_db(chat: *mut Chat, chat_id: u32) -> bool {
    ...
    let res = context.sql.query_row(
        ...
        |row| {
            ...
            let packed = CString::new(row.get::<_, String>(4)?).unwrap();
            unsafe { dc_param_set_packed((*chat).param, packed.as_ptr()) };
...
unsafe fn prepare_msg_raw(
        ...
        && 0 == dc_is_contact_in_chat(context, (*chat).id, 1 as uint32_t)
    {
        ...
        if from.is_none() {
            ...
            let from_c = CString::new(from.unwrap()).unwrap();
...
unsafe fn last_msg_in_chat_encrypted(
    ...
    if let Some(packed) = packed {
        ...
        let packed_c = CString::new(packed).unwrap();
