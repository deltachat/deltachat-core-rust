...
pub async fn forward_msgs(context: &Context, msg_ids: &[MsgId], chat_id: ChatId) -> Result<()> {
    ...
    if let Ok(mut chat) = Chat::load_from_db(context, chat_id).await {
        ...
        for id in ids {
            ...
            // Set the subject to Fwd: <previous subject>, if this makes sense
            // but do not leak data as group names, and do not forward subjects from DC messages as
            // they usually don't contain any information anyway
            let subject_contains_group_name =
                chat.typ == Chattype::Group && msg.subject.contains(&chat.name);
            if msg.is_dc_message == MessengerMessage::No && !subject_contains_group_name {
                msg.subject = format!("Fwd: {}", remove_subject_prefix(&msg.subject));
            } else {
                // a default subject is generated by mimfactory
