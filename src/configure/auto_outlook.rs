...
fn parse_xml(xml_raw: &str) -> Result<ParsingResult, Error> {
    ...
    {
        if outlk_ad.out.srv_params[IDX_IMAP].hostname.is_empty()
            || outlk_ad.out.srv_params[IDX_IMAP].port == 0
            || outlk_ad.out.srv_params[IDX_SMTP].hostname.is_empty()
            || outlk_ad.out.srv_params[IDX_SMTP].port == 0
        {
    ...
    if tag == "protocol" {
        if let Some(type_) = &outlk_ad.config_type {
            ...
            if type_ == "imap" && !outlk_ad.out_imap_set {
                outlk_ad.out.srv_params[IDX_IMAP].hostname =
                    std::mem::replace(&mut outlk_ad.config_server, String::new());
                outlk_ad.out.srv_params[IDX_IMAP].port = port;
                if ssl_on {
                    outlk_ad.out.srv_params[IDX_IMAP].security = Some(ServerSecurity::Ssl);
                } else if ssl_off {
                    outlk_ad.out.srv_params[IDX_IMAP].security = Some(ServerSecurity::PlainSocket);
                }
                outlk_ad.out_imap_set = true
            } else if type_ == "smtp" && !outlk_ad.out_smtp_set {
                outlk_ad.out.srv_params[IDX_SMTP].hostname =
                    std::mem::replace(&mut outlk_ad.config_server, String::new());
                outlk_ad.out.srv_params[IDX_SMTP].port = outlk_ad.config_port;
                if ssl_on {
                    outlk_ad.out.srv_params[IDX_SMTP].security = Some(ServerSecurity::Ssl);
                } else if ssl_off {
                    outlk_ad.out.srv_params[IDX_SMTP].security = Some(ServerSecurity::PlainSocket);
                }
      ...
      <Protocol>
        ...
        match res {
            ParsingResult::LoginParam(lp) => {
                assert_eq!(lp.srv_params[IDX_IMAP].hostname, "example.com");
                assert_eq!(lp.srv_params[IDX_IMAP].port, 993);
                assert_eq!(lp.srv_params[IDX_SMTP].hostname, "smtp.example.com");
                assert_eq!(lp.srv_params[IDX_SMTP].port, 25);
            }
