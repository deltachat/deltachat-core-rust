...
///     are returned.
pub fn dc_get_chatlist<'a>(
    context: &'a Context,
    listflags: usize,
    query: Option<&str>,
    query_contact_id: Option<u32>,
) -> Result<dc_chatlist_t<'a>> {
...
pub fn dc_chatlist_get_cnt(chatlist: &dc_chatlist_t) -> usize {
    chatlist.ids.len()
}
...
pub fn dc_chatlist_get_chat_id(chatlist: &dc_chatlist_t, index: usize) -> u32 {
    if index >= chatlist.ids.len() {
        return 0;
    }
    chatlist.ids[index].0
}

/// Get a single message ID of a chatlist.
///
/// To get the message object from the message ID, use dc_get_msg().
pub fn dc_chatlist_get_msg_id(chatlist: &dc_chatlist_t, index: usize) -> u32 {
    if index >= chatlist.ids.len() {
        return 0;
    }

    chatlist.ids[index].1
}
...
pub unsafe fn dc_chatlist_get_summary<'a>(
    ...
    if (*chat).id == DC_CHAT_ID_ARCHIVED_LINK as u32 {
        ...
        (*ret).text2 = to_cstring(chatlist.context.stock_str(StockMessage::NoMessages));
    } else {
