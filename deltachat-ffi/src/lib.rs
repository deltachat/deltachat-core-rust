...
///   to "Delta Chat Core <version>".  This must be **valid UTF-8**
///   and will be lossy converted if it is not.
///
...
/// * `context` - The context object as created by [dc_context_new].
///
/// * `dbfile` - The file to use to store the database, something like
///   `~/file` won't work on all systems, if in doubt, use absolute
///   paths.
///
/// * `blobdir` - A directory to store the blobs in; a trailing slash
///   is not needed.  If you pass NULL or the empty string,
///   deltachat-core creates a directory beside _dbfile_ with the same
///   name and the suffix `-blobs`.
///
/// # Returns
///
...
///
/// Before the context is closed, connections to SMTP and IMAP are
/// closed; these connections are started automatically as needed
/// eg. by sending for fetching messages.  This function is also
/// implicitly called by [dc_context_unref].  Multiple calls to this
/// functions are okay, the function takes care not to free objects
/// twice.
///
/// This will **block** if there are still objects referencing the
/// context like [dc_chatlist_t], [dc_chat_t], [dc_msg_t] or
/// [dc_contact_t].  This is because these objects keep a read-lock on
/// the context and all these locks must be released before the
/// context can be closed.  Thus make sure to call the appropriate
/// `*_unref()` functions on all objects before closing the context.
///
/// @memberof [dc_context_t]
///
...
/// used "as is" therefore, [contact::normalize_name] is _not_ called
/// for the name. If the contact is blocked, it is unblocked.
///
